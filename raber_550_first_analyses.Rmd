...
# Running the below code with Sex_Treatment yielded an R2 of nearly 10%, but this is a post-hoc observation that isn't necessarily meaningful
# Look at how this runs on rarefied data instead of the crap you're feeding it now
set.seed(1)
perm.results.treatment <- adonis(physeq1.dist ~ OFTotDistDay1, data=sampledf, permutations=5000)
...
## I can't get the data into a df for post-hoc analysis because the columns are different lengths as a result of trimming samples earlier, so see Tukey below and talk with Tom
#beta_disper_data_frame <- data.frame(group=c("group_0","group_25"), distance= c(beta_disp_0, beta_disp_25))
#wilcoxon_results <- wilcox.test(distances ~ group, data=beta_distper_data_frame)
