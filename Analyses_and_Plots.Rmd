...
alphadiv_lm <- lm(as.matrix(Alphadiv) ~ FCCuedFrzTone, data=anova_df)
plot(alphadiv_lm)
aov_results <- aov(as.matrix(Alphadiv) ~ Treatment, data=anova_df)
anova_results <- anova(aov_results)
some_plot <- plot((as.matrix(Alphadiv))~Treatment, data=anova_df)
print(anova_results)
testing_cor_adiv <- cor.test(x=anova_df$FSTTimeImmobile, y=(as.matrix(anova_df$Alphadiv)), method=c("kendall"))
...
```
```{r commentary}
##Alpha diversity is a metric that describes the diversity of taxa in a specific sample or set. Mean alpha diversity measures did not differ significantly between the treatment groups. Of the behavioral covariates and no association was detected. However, FSTTimeImmobile times were associated with alpha diversity (alpha diversity metric "Shannon", Kendall's tau, p-value=0.03692).
...
##There were associations between covariates of interest and taxa. DESeq2 was used to analyze the association between bacterial taxa and study covariates. Most associations were at the ASV level.
```
