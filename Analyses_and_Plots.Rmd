...
cont_treatment <- sample_data(physeq)$Treatment
...
ggplot(corr_df, aes(x=FSTTimeImmobile, y=as.matrix(Alphadiv)), ylab="Shannon") + geom_smooth(method="lm", ylab="Shannon") + geom_point(aes(color=FSTTimeImmobile_binned)) + ylab("Shannon") + xlab("Forced Swim Test Immobility Time") + theme(legend.position = "none") 
...
library("phyloseq")
physeq1.ord <- ordinate(physeq1, "NMDS", "bray", trymax=500, k=3)
physeq1.ord.plot <- plot_ordination(physeq1, physeq1.ord, type="samples", color="Sex")
plot(physeq1.ord.plot)
physeq1.ord.treatment <- plot_ordination(physeq1, physeq1.ord, type="samples", color="Treatment", shape="Sex", title="") + stat_ellipse(mapping = aes(linetype=Sex))
...
physeq_nmds = metaMDS(otu_table(physeq1), distance="jaccard", k=3, trymax=500, data=treatment.env)
...
physeq1.dist <- vegdist(otu_table(physeq1), method="canberra")
...
taxa_hist <- ggplot(data=taxa_table_plus_metadata_asv_rare, aes(ASV231, stat(count))) +
  geom_histogram(aes(fill=I("lightgrey"), col=I("white")), bins = 5) +
  theme_bw() +
...
##ASVs: 316 (not monotonic, but associated with quartiles above 1), 302 (looks better unbinned), 426, 
