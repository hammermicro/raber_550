...
physeq1_ord_treatment <- plot_ordination(physeq1, physeq1.ord, type="samples", color="Treatment", shape="Sex", title="") + stat_ellipse(mapping = aes(linetype=Sex))
...
```{r combining_figures}
library(gridExtra)
multi_figure <- grid.arrange(FST_by_Shannon_plot, physeq1_ord_treatment, taxa_hist, taxa_hist)
```

```{r taxa_list}

##FSTTimeImmobile - ASV315 (bin=4)
##NOObjpref - ASV417 (bin=3|4)
##OFTotDistDay1 - ASV316 (bin=4)
##PALatencyDiff - ASV302 (bin=4), ASV506 (bin=3|4)
##PALatCross = ASV302 (bin=3|4), ASV437 (a bit weak)
##
