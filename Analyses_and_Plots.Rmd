...
##plotting_taxa <- ggplot(taxa_table_plus_metadata_genus_rare, aes(x=Treatment, y=ASV656))  + geom_smooth(method="lm") + geom_jitter()
##plotting_taxa

## ASV429 from the Ruminococceae was identified as being significantly associated with Treatment(p-value=) and the graph of this shows high abundance for a number of samples at a low radiation level, but none at 50cGy and above.
##plotting_taxa <- ggplot(taxa_table_plus_metadata_asv_rare, aes(x=Treatment, y=ASV429)) + geom_jitter() + geom_smooth(method="lm")
##plotting_taxa

##plotting_taxa <- ggplot(taxa_table_plus_metadata_asv_rare, aes(x=Sex, y=ASV6)) + geom_boxplot() + facet_grid(cols=vars(Treatment), as.table=TRUE)
##plotting_taxa
##plotting_taxa <- ggplot(taxa_table_plus_metadata_asv_rare, aes(x=Treatment, y=ASV231)) + geom_point() + geom_smooth(method="auto") + facet_grid(rows=vars(Sex))
##plotting_taxa
##ASVs to consider: 
```
...
##ASVs: 302 (visit with Tom about this one), 

##plotting_taxa <- ggplot(taxa_table_plus_metadata_asv_rare, aes(x=PALatencyDiff, y=ASV302))  + geom_point(aes(color=Treatment))
##plotting_taxa
```
