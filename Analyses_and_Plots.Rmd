...
resultsofstuff <- wilcox.test(alphadiv ~ Radiated, data=some_sample_data)
...
alphadiv_lm <- lm(as.matrix(Alphadiv) ~ Treatment, data=anova_df)
plot(alphadiv_lm)
...
physeq1.ord.plot.treatment <- plot_ordination(physeq1, physeq1.ord, type="samples", color="Sex_Treatment", title="BySex_Treatment") + stat_ellipse(level=0.95)##optional parameter for any of the above plots + stat_ellipse(level=0.4)
...
taxa_table_plus_metadata_genus <- data.frame(cbind(data.frame(asv_table), data.frame(sample_data_to_smash)))
...
plotting_taxa <- ggplot(taxa_table_plus_metadata_genus, aes(x=Treatment, y=ASV656))  + geom_smooth() + geom_jitter()
plotting_taxa
...
plotting_taxa <- ggplot(taxa_table_plus_metadata_asv, aes(x=Treatment, y=ASV429)) + geom_jitter()
plotting_taxa
...
```
```{r commentary}
##Alpha diversity is a metric that describes the diversity of taxa in a specific sample or set. Mean alpha diversity measures did not differ significantly between the analyze treatment groups (Treatment Pr(>F)~0.129, Treatment interaction with Sex, Pr(>F)~0.061).The compositional diversity between samples, beta-diversity, was first estimated using the Bray-Curtis dissimilarity index, and a NMDS ordination was created. From this information, an analog to step-wise AIC was used to elucidate the best model for predicting differences in beta-diversity. PERMANOVA was performed using Treatment x Sex to quantify the difference in beta diversity between samples. The results of this test suggest that the composition of the gut changes in response to Treatment interacting with Sex (p-value ~0.0006, R2~0.041). While no association was found between beta-diversity and other behavioral covariates, there were associations with some individual taxa. DESeq2 was used to analyze the association between bacterial taxa and study covariates.
```
